...
d3.json("../tests/data/demo.json", function(error, graph) {
  if (error) throw error;

  var verticies = [];
  for (var i in graph.nodes) {
    for (var j in graph.nodes) {
      if (graph.nodes[j].id == graph.nodes[i].head) {
        verticies.push({"source":graph.nodes[i].form, "target":graph.nodes[j].form});
        break;
      }
    }
  }
